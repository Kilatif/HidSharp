...
namespace HidSharp.Platform.Windows
{
    sealed class WinSerialStream : SysSerialStream
    {
        ...
        internal void Init(string devicePath)
        {
            ...
            if (handle == (IntPtr)(-1))
            {
                throw DeviceException.CreateIOException(Device, "Unable to open serial device (" + devicePath + ").");
            }

            var timeouts = new NativeMethods.COMMTIMEOUTS();
            timeouts.ReadIntervalTimeout = uint.MaxValue;
            timeouts.ReadTotalTimeoutConstant = uint.MaxValue;
            timeouts.ReadTotalTimeoutMultiplier = uint.MaxValue;
            if (!NativeMethods.SetCommTimeouts(handle, out timeouts))
            {
                NativeMethods.CloseHandle(handle);
                throw DeviceException.CreateIOException(Device, "Unable to set serial timeouts.");
            }
        ...
        void UpdateSettings()
        {
            lock (_lock)
            {
                ...
                if (!NativeMethods.GetCommState(_handle, ref dcb)) { throw DeviceException.CreateIOException(Device, "Failed to get serial state."); }

                dcb.BaudRate = checked((uint)_baudRate);
                dcb.ByteSize = 8;
                dcb.Parity = NativeMethods.NOPARITY;
                dcb.StopBits = NativeMethods.ONESTOPBIT;
                dcb.fFlags = 0;
                dcb.fBinary = true;
                if (!NativeMethods.SetCommState(_handle, ref dcb)) { throw DeviceException.CreateIOException(Device, "Failed to set serial state."); }
                ...
                if (!NativeMethods.PurgeComm(_handle, purgeFlags)) { throw DeviceException.CreateIOException(Device, "Failed to purge serial port."); }
            }
        }
        ...
        public sealed override int BaudRate
        {
            ...
            set
            {
                lock (_lock)
                {
                    ...
                    _baudRate = value; _settingsChanged = true;
                }
            }
        }
